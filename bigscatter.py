...
for i in range(len(columns)):
	for x in range(len(metrics)):
		axes[i][x].scatter(aggregate[columns[i]], numbered[metrics[x][0]])
		axes[i][x].set_xlabel(columns[i] + "(aggr)")
		axes[i][x].set_ylabel(metrics[x][1])
		spear = pd.DataFrame(aggregate[columns[i]])[0].corr(pd.DataFrame(numbered[metrics[x][0]])[metrics[x][0]], method='spearman')
		axes[i][x].set_title("Spearman: " + str(spear))

#tighten layout
#fig.suptitle("Aggregates for File " + str(file_num))
plt.tight_layout()
plt.savefig("aggr_scatter_all.png")
...
for i in range(len(columns)):
	for x in range(len(metrics)):
		axes[i][x].scatter(average[columns[i]], numbered[metrics[x][0]])
		axes[i][x].set_xlabel(columns[i] + "(avg)")
		axes[i][x].set_ylabel(metrics[x][1])
		spear = pd.DataFrame(average[columns[i]])[0].corr(pd.DataFrame(numbered[metrics[x][0]])[metrics[x][0]], method='spearman')
		axes[i][x].set_title("Spearman: " + str(spear))


#fig.suptitle("Averages for File " + str(file_num))
plt.tight_layout()
plt.savefig("avg_scatter_all.png")